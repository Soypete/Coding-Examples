				...
				if (die_values_array[k] == 6){
					...
					for (int j = 0; j < 5; j++){												//looks through everyvalue of the array
						if (die_values_array[j] == 6){
							...
							return true;					//change to value to a non comparable so that it will not get compared to itself
						}
						else{ return false; }
...
int main(){
	/*bool truth = isValidOption_YAHTZEE(3,3,3,3,3);
	if (truth == true){
		cout << "true";
	}
	else {
	...
	/*bool truth = isValidOption_SmStraight(5,2,1,4,6);
	if (truth == true){
		cout << "true";
	}
	else {
	...
	bool truth = isValidOption_3OfAKind(1, 1, 1, 1, 1);
	if (truth == true){
		cout << "true";
	}
	else {
